...
let rec subaux (l : 'a list)(m : 'a list) : bool = match l, m with (*Funzione di supporto per sublist*)
    [], _ -> true
  | _, [] -> false
  | hdl::tll, hdm::tlm -> hdl = hdm && subaux tll tlm;; 

let rec sublist (l : 'a list)(m : 'a list) : bool = match l, m with (*Funzione per verificare che una lista sia contenuta in un'altra*)
    [], _ -> true
  | _, [] -> false
...
let rec list_as_set (l : 'a list) : 'a list = match l with (*Funzione che rimuove i duplicati da una lista*)
    [] -> []
  ...
  | IsPositive a -> ispositive (eval a r) (*Funzione aggiuntiva che restituisce Bool(true) se a >= 0*)
      ...
      (match fClosure with
       ...
       |	_ -> failwith("non functional value")) 
		...
		| Empty(type_) ->( match type_ with (*Inizializza l'insieme vuoto di tipo "type_"*)
        "int" -> SetVal([], type_)
      | "bool" -> SetVal([], type_)
      | "string" -> SetVal([], type_)
      | _ -> failwith("Not a valid type"))
      ...
      else failwith("Wrong type") (*Inizializza un insieme di un elemento di tipo "type_"*)
